...
exports.addItem = async (req, res, next) => {
  try {
    ...
    if (!Number.isFinite(nQty) || nQty < 1) throw new ApiError(400, 'qty must be >= 1');

    const product = await Product.findById(productId);
    if (!product || product.isActive === false) throw new ApiError(404, 'Product not found');
    if (product.stock < nQty) throw new ApiError(400, 'Insufficient stock');
...
exports.updateItemQty = async (req, res, next) => {
  try {
    ...
    item.imageSnapshot = product.images?.[0] || null;
